...
class JiraOperator:
    ...
    def turnaroundTime(self, issueNo):
        ...
        url = self.jiraUrl + "/rest/api/2/issue/{}?expand=changelog".format(issueNo)
        response = requests.get(url, headers=headers, verify=False,
                                auth=HTTPBasicAuth(self.jiraUser, self.jiraPwd))
        if response.status_code == 200:
            self.issueCreated = response.json()["fields"]["created"]
            print(issueNo + " was created at " + self.issueCreated)
            ...
            for history in histories:
                if "8.0 Current Release" == history["items"][-1]["toString"]:
                    ...
                    print(issueNo + " was released at " + history["created"])
                    break
                if "Closed" == history["items"][-1]["toString"]:
                    self.issueDone = history["created"]
                    print(issueNo + " was closed at " + history["created"])
                    break
    ...
    def turnaroundTimeBatch(self, issueList):
        for jiraIssue in issueList:
            jiraIssue.turnaroundTime = self.turnaroundTime(jiraIssue.key)

    def idleTime(self, issueNo):
        self.issueCreated = ""  # reset the values
        ...
        if response.status_code == 200:
            ...
            for history in histories:
                if ("2.0 In Progress" == history["items"][-1]["toString"]):
                    ...
                    print(issueNo + " was in progress at " + history["created"])
                    break
                if "Closed" == history["items"][-1]["toString"]:
                    self.issueStarted = history["created"]
                    print(issueNo + " was closed at " + history["created"])
    ...
    def idleTimeBatch(self, issueList):
        for jiraIssue in issueList:
            jiraIssue.idleTime = self.idleTime(jiraIssue.key)
    ...
    def jqlSearch(self, jqlString):
        ...
        jiraIssues = []
        for issue in issues:
            ji = JiraIssue(issue["key"], self.turnaroundTime(issue["key"]))
